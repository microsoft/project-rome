...
import com.microsoft.connecteddevices.core.ApplicationRegistration;
...
import com.microsoft.connecteddevices.core.ApplicationRegistrationBuilder;
...
public class MainActivity
  ...
  private final static Logger LOG =
      Logger.getLogger(MainActivity.class.getSimpleName());
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    mProximalDiscoveryCheckbox =
        (CheckBox)findViewById(R.id.chkProximalDiscovery);
    ...
    if ((ACTION_SEND == launchIntent.getAction()) ||
        (ACTION_SEND_MULTIPLE == launchIntent.getAction())) {
      switch (launchIntent.getAction()) {
      case ACTION_SEND: {
        mFiles =
            new Uri[] {launchIntent.getParcelableExtra(Intent.EXTRA_STREAM)};
        break;
      }
      case ACTION_SEND_MULTIPLE: {
        ArrayList<Uri> files =
            launchIntent.getParcelableArrayListExtra(Intent.EXTRA_STREAM);
   ...
   * This is required before we attempt to use Project Rome SDK.
   */

  private void initializePlatform() {
    ApplicationRegistrationBuilder registrationBuilder =
        new ApplicationRegistrationBuilder();
    ApplicationRegistration registration =
        registrationBuilder.buildRegistration();

    AsyncOperation<PlatformCreationResult> platformCreationResult =
        Platform.createInstanceAsync(getApplicationContext(), null,
                                     registration, null);

    platformCreationResult.whenCompleteAsync(
        new AsyncOperation
            .ResultBiConsumer<PlatformCreationResult, Throwable>() {
              @Override
              public void accept(PlatformCreationResult platformCreationResult,
                                 Throwable throwable) throws Throwable {
                if (throwable != null) {
                  LOG.log(Level.SEVERE, "Failed to initialized platform");
                } else {
                  if (PlatformCreationStatus.FAILURE ==
                      platformCreationResult.getStatus()) {
                    LOG.log(Level.SEVERE, "Failed to initialized platfrom");
                  } else {
  ...
  /**
   * This method starts the RemoteSystem discovery process. It sets the
   * corresponding filters
   * to ensure that only spatially proximal devices are listed. It also sets up
   * listeners
   * for important events, such as device added, device updated, and device
   * removed
   */
  ...
  private void startDiscovery() {
    try {
      ...
      if (mProximalDiscoveryCheckbox.isChecked()) {
        filters.add(new RemoteSystemDiscoveryTypeFilter(
            RemoteSystemDiscoveryType.PROXIMAL));
      } else {
        filters.add(new RemoteSystemDiscoveryTypeFilter(
            RemoteSystemDiscoveryType.SPATIALLY_PROXIMAL));
      }

      filters.add(new RemoteSystemStatusTypeFilter(RemoteSystemStatusType.ANY));
      filters.add(new RemoteSystemAuthorizationKindFilter(
          RemoteSystemAuthorizationKind.ANONYMOUS));

      mRemoteSystemWatcher = new RemoteSystemWatcher(
          filters.toArray(new RemoteSystemFilter[filters.size()]));

      mRemoteSystemWatcher.addRemoteSystemAddedListener(
          new RemoteSystemAddedListener());
      mRemoteSystemWatcher.addRemoteSystemUpdatedListener(
          new RemoteSystemUpdatedListener());
      mRemoteSystemWatcher.addRemoteSystemRemovedListener(
          new RemoteSystemRemovedListener());
      mRemoteSystemWatcher.addErrorOccurredListener(
          new RemoteSystemErrorListener());
      ...
      LOG.log(Level.SEVERE,
              String.format("Discovery failed: %1$s", exception.getMessage()));
    }
  }

  /**
   * Request COARSE_LOCATION permission required for nearshare functionality
   * over bluetooth.
   */

  private void requestPermissions() {
    // Request user permission for app to use location services, which is a
    // requirement for Bluetooth.
  ...
  private void sendUri() {
    ...
    RemoteSystemConnectionRequest remoteSystemConnectionRequest =
        new RemoteSystemConnectionRequest(mSelectedRemoteSystem);
  ...
  /**
   * Send file(s) to the target device using nearshare. This functionality
   * requires the nearshare app to be used as a share target. The
   * operation starts with
   * an app such as the photos app. Select a photo and share to the nearshare
   * app.
   */
  ...
  private void sendFile() {
    if ((null != mFiles) && (null != mSelectedRemoteSystem)) {
      ...
      if (mNearShareSender.isNearShareSupported(
        ...
        // Call the appropriate api based on the number of files shared to the
        // app.
        if (1 == mFiles.length) {
          NearShareFileProvider nearShareFileProvider =
              NearShareHelper.createNearShareFileFromContentUri(
                  mFiles[0], getApplicationContext());

          asyncFileTransferOperation = mNearShareSender.sendFileAsync(
              remoteSystemConnectionRequest, nearShareFileProvider,
              progressCallback);
        } else {
          NearShareFileProvider[] nearShareFileProviderArray =
              new NearShareFileProvider[mFiles.length];

          for (int index = 0; index < mFiles.length; ++index) {
            nearShareFileProviderArray[index] =
                NearShareHelper.createNearShareFileFromContentUri(
                    mFiles[index], getApplicationContext());
          }

          asyncFileTransferOperation = mNearShareSender.sendFilesAsync(
              remoteSystemConnectionRequest, nearShareFileProviderArray,
              progressCallback);
        }
        ...
        ((Button)findViewById(R.id.btnCancel))
            .setOnClickListener(new View.OnClickListener() {
              ...
              private View.OnClickListener init(
                  AsyncOperation<NearShareStatus> asyncOperation) {
        ...
        asyncFileTransferOperation.whenCompleteAsync(
            new AsyncOperation.ResultBiConsumer<NearShareStatus, Throwable>() {
              @Override
              public void accept(NearShareStatus nearShareStatus,
                ...
                if (null != throwable) {
                  LOG.log(Level.SEVERE,
                          String.format("Exception during file transfer: %1$s",
  ...
  public void onItemClick(AdapterView<?> parent, View view, int position,
                          long id) {
    mSelectedRemoteSystem =
        (RemoteSystem)mRemoteDeviceListAdapter.getItem(position);
    mRemoteDeviceListAdapter.setSelectedView(view);
  }

  // region HelperClasses

  private class RemoteSystemAddedListener
      implements EventListener<RemoteSystemWatcher, RemoteSystem> {
    @Override
    public void onEvent(RemoteSystemWatcher remoteSystemWatcher,
  ...
  private class RemoteSystemUpdatedListener
      implements EventListener<RemoteSystemWatcher, RemoteSystem> {
    @Override
    public void onEvent(RemoteSystemWatcher remoteSystemWatcher,
                        RemoteSystem remoteSystem) {
      LOG.log(Level.INFO, String.format("Updating system: %1$s",
                                        remoteSystem.getDisplayName()));
    }
  }

  private class RemoteSystemRemovedListener
      implements EventListener<RemoteSystemWatcher, RemoteSystem> {
    @Override
    public void onEvent(RemoteSystemWatcher remoteSystemWatcher,
                        RemoteSystem remoteSystem) {
  ...
  private class RemoteSystemErrorListener
      implements EventListener<RemoteSystemWatcher, RemoteSystemWatcherError> {
    @Override
    public void onEvent(RemoteSystemWatcher remoteSystemWatcher,
                        RemoteSystemWatcherError remoteSystemWatcherError) {
      LOG.log(Level.INFO, String.format("Discovery error: %1$s",
                                        remoteSystemWatcherError.toString()));
    }
  }
  // region HelperClasses
}
